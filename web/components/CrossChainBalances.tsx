 ...
 * Backed by app/api/portfolio/balances, which reads the FXRP ERC-20 on Flare
 * directly (the OFT Adapter there holds no user balances) and the native
 * LayerZero OFT contracts on every other chain in lib/oftDeployments.ts.
 *
...
export function CrossChainBalances() {
  ...
  async function loadAll() {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/portfolio/balances?address=${input}`, { cache: "no-store" });
  ...
  return (
    <section className="bg-zinc-900/30 border border-zinc-800/80 backdrop-blur-md p-6 rounded-2xl shadow-xl">
      ...
      <p className="text-zinc-400 text-sm mt-1 mb-4">
        FXRP balance on Flare plus every chain where the LayerZero OFT is deployed.
      </p>
